...
ALLOW_EXECUTION_ENV = "CYBERSEC_SKILL_ALLOW_ATOMIC_EXECUTION"
...
def atomic_execution_allowed():
    """Require an explicit opt-in before running Atomic Red Team commands."""
    return os.environ.get(ALLOW_EXECUTION_ENV) == "1"


def execution_blocked_response(action):
    return {
        "status": "blocked",
        "action": action,
        "message": f"Set {ALLOW_EXECUTION_ENV}=1 to run Atomic Red Team commands.",
    }
...
def execute_atomic_test(atomics_path, technique_id, test_number=1, platform="linux"):
    ...
    if not atomic_execution_allowed():
        return execution_blocked_response("execute")
...
def run_cleanup(atomics_path, technique_id, test_number=1):
    ...
    if not atomic_execution_allowed():
        return execution_blocked_response("cleanup")
