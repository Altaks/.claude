...
4. **Identity**: Anonymous or authenticated? If the bugs you're hunting need a
   ...
   pass the target's auth header (`-H 'Cookie: …'` / `-H 'Authorization: Bearer …'`)
   to each MCP `run_tool` call (httpx, katana, ffuf, nuclei, dalfox, PoC verifiers).
   The MCP audit log redacts credential-shaped strings before writing.
