...
> `SecretFinder`, `LinkFinder`, and `GitDorker` are **not in `tools_config.json`**.
> Add them with the `add-tool` skill (git clone into `~/tools/`) before the commands
> below resolve, or substitute an MCP-native pass: `run_pipeline` of
> `curl <js-url> | grep -oE "(api[_-]?key|secret|token)['\"]?\\s*[:=]\\s*['\"][^'\"]+"`.
